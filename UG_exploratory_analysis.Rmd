...
Notably, the total UG area in NYC sums to `r sum(UG_df$green_area)` square feet, being `r mean(UG_df$green_area)` square feet the average size of UGs. When assessing the proportion that each UG covers in relation to the total building area, an average `r mean(UG_df$perc_gr)`% of the affected buildings are covered in such.

When stratified by borough, differences in the amount of UGs as well as other indicators become apparent. Key variables are summarized in the table below using `knitr:kable()`.
...
```{r}
UG_map <- leaflet(UG_df) %>% 
  addTiles() %>% 
  addMarkers(~longitude, ~latitude, popup = ~ UG_id)

UG_map
```

**Note**: using indications on the code from [this](https://teachdatascience.com/leaflet/) website.They're very simple. On a second iteration, I intend to use the code provided by the authors referenced below (find separate .Rmd in the repo).
