---
title: "Earned Income Tax Data (EITC) and Population"
author: "Emma Warshaw"
date: "2022-11-30"
...
```
We're interested in understanding the financial status of residents in each borough, ideally to understand differences in relation to rooftop greenspace in each borough.  
...
The Noncustodial Parent EITC is offered exclusively by the state and claimants can only file for this credit alone.  If they claim Noncustodial Parent EITC, it disqualifies them from regular EITC eligibility.  
...
Finally, the `credit_amount_claimed_in_thousands` needs to be multiplied by 1000 to get the actual dollar amount claimed in each borough.  I'm also going to rename `county` to `borough`for consistency across the project.   
...
## Hypothesis Testing: Proportion of Claimants
...
First we'll run an F-test to determine if the group proportions are different between boroughs.  For purposes of this project, we're using Manhattan as our universal reference group, so we'll then see which proportions differ from Manhattan.     
...
## Hypothesis Testing: Average Credit Amount

Similar to the population proportions, we need to test if these difference are significant. 

We'll start with the EITC data and run an F-test to determine if the amount claimed is different in between boroughs.  We'll then use Manhattan as our reference group to see which boroughs are paying a different amount. 
...
The F-test gives a p-value of 0.4237.  For Noncustodial EITC, there is no statistical difference in the average amount of credit received among the 5 boroughs.   
...
Moving on to average dollar amount of the credit in each borough, there's no statstical difference in Noncustodial EITC among the boroughs.  
...
## Relationship Between Tax Data and Greenspace

When trying to find some association between greenspace data and average EITC tax credit amount, it's not possibly to draw statistical conclusions with the information we have.  That being said, we can create visualizations that give an overall idea of trends.  
