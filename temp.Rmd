...
### Univariate Models to Explore the Effect of Total Average Urban Green Spaces in NYC
...
envo_greenroof_surfmodel = 
  lm(avg_daytime_temp ~ avg_total_green_area, data = envo_greenroof) %>% 
  tidy() %>% 
  select(-statistic) %>% 
  mutate(
    low_conf = estimate - 1.96*std.error,
...
envo_greenroof_surfmodel %>% 
  knitr::kable(digits = 8, align = "lcccc", caption = "Table 6: Coefficient Estimates of Linear Model Assessing the Association Between Average Green Roof Surface Area and Average Daytime Surface Temperature", col.names = c("Term", "Estimate", "p-value", "Lower Confidence Interval", "Upper Confidence Interval"))
```
...
envo_greenroof_heatmodel = 
  lm(avg_heat_index ~ avg_total_green_area, data = envo_greenroof) %>% 
  tidy() %>% 
  select(-statistic) %>% 
  mutate(
    low_conf = estimate - 1.96*std.error,
...
envo_greenroof_heatmodel %>% 
  knitr::kable(digits = 8, align = "lcccc", caption = "Table 7: Coefficient Estimates of Linear Model Assessing the Association Between Average Green Roof Surface Area and Average Heat Vulnerability Index", col.names = c("Term", "Estimate", "p-value", "Lower Confidence Interval", "Upper Confidence Interval"))
```
